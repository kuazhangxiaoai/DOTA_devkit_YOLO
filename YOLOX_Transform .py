...
def drawLongsideFormatimg(imgpath, txtpath, dstpath, extractclassname, thickness=2):
    ...
    #filelist = util.GetFileFromThisRootDir(txtpath)  # fileist=['/.../P0005.txt', ..., /.../P000?.txt]
    with open(txtpath,"r") as f_out:
        lines = f_out.readlines()
        filelist = [path.split(" ")[0] for path in lines]
        polylist =[]
        clslist = []
        thelta_list = []
        for line in lines:
            objects = line.strip().split(" ")[1:]
            polys = [list(map(int,obj.split(",")[:4])) for obj in objects]
            cls = [int(obj.split(",")[-2]) for obj in objects]
            thelta_longside = [int(obj.split(",")[-1]) for obj in objects]
            polylist.append(polys)
            clslist.append(cls)
            thelta_list.append(thelta_longside)

    for i, fullname in enumerate(filelist):
        assert len(filelist) == len(polylist)
        assert len(filelist) == len(clslist)
        assert len(filelist) == len(thelta_list)
        polys, clses, thelta_longside = polylist[i], clslist[i], thelta_list[i]
        name = os.path.splitext(os.path.basename(fullname))[0] # name='P000?'
        img_fullname = os.path.join(imgpath, name + '.png')  # img_fullname='/.../P000?.png'
        img_savename = os.path.join(dstpath, name + '_.png')  # img_fullname='/.../_P000?.png'
        img = cv2.imread(img_fullname)  # 读取图像像素
        for j, poly in enumerate(polys):
            assert len(polys) == len(clses)
            assert len(polys) == len(thelta_longside)
            clsIndex = clses[j]
